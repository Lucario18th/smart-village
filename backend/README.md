...
- **Registration:** `POST /api/auth/register` erfordert jetzt `postalCodeId` (ID aus obiger Suche). Accounts werden Gemeinden zugeordnet, die an eine gültige deutsche PLZ gebunden sind.
- **Villages:** Village-Antworten enthalten `postalCode` Relation; `PUT /api/villages/:id` akzeptiert `postalCodeId` zum Aktualisieren.
