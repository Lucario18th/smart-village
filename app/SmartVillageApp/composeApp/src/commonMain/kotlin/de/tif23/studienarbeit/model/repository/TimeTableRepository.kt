...
import de.tif23.studienarbeit.model.data.responses.TimeTableChangesResponse
...
class TimeTableRepository {
    ...
    suspend fun getTimetableChanges(evaNo: String): TimeTableChangesResponse {
        val result = client.get("$DB_TIMETABLES_API_URL/fchg/$evaNo") {
            headers {
                append(HttpHeaders.Accept, "application/xml")
                append("DB-Client-ID", BuildKonfig.DB_CLIENT_ID)
                append("DB-Api-Key", BuildKonfig.DB_CLIENT_SECRET)
        ...
        val xmlResponse = result.bodyAsText()
        println(xmlResponse)
        return XML.v1.decodeFromString(TimeTableChangesResponse.serializer(), xmlResponse)
    }
}